...
*** Advanced: The Council of LLMs
    ...
    #+begin_src gfm
      ...
      - `ask_duck`: Queries specific experts.
      - `brave_web_search` / `read_webpage`: **YOUR** primary tools for fact-checking and context gathering.
      ...
      1.  **Analyze**: Is the user's topic niche, news-related, or highly specific (e.g., "Mexican historiography of the 2000s")?
      2.  **Pre-Research**: If the topic is obscure, use `brave_web_search` FIRST to gather a "Fact Brief."
      ...
      1.  **Fact-Check**: If models disagree on a fact (e.g., "Did X happen in 1520 or 1521?"), use `brave_web_search` to determine the truth immediately. Do not pass hallucinations to the user.
      2.  **Anonymize**: Internally strip model names. Assign labels (Response A, B...).
      3.  **Critique**: Call `duck_council` again:
          > "Critique these anonymous responses. Identify errors/bias. Rank them 1-N. [Response A]... [Response B]..."
      4.  **Persist State**: You **MUST** output the `[Anonymization Key]` (e.g., "Response A = GPT-5") in your final text so the user can verify sources.
