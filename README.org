...
* Update early 2026
  ...
  One True precise system prompt is not nearly so important as it was way back in the aulden days of 2025 and before.

  When I started this project, I was struggling with implementing *composable* prompts with segments that could be concatenated for specific projects.

  Then along came MCP servers.

  And then *skills*.

  Skills are prompts - composable not by you the user, but by the LLM.  Every AI platform supports them now, and if yours does not, you can simply tell the LLM where you have some skills in a directory, and the LLM will figure out how to use them.  They're dead simple, and they often leverage either simple MCP servers and/or CLI scripts or APIs.  They do not impose the "LLM cognitive overload" that many MCP servers do, so you end up with an uncluttered context, which means a clear-thinking LLM.  I have just started implementing some skills here, including wrapper-first Home Assistant control and a standalone =spotify-cli= skill that tracks the evolving local CLI surface.  The reality is that you can find huge lists of useful skills all over the internet.  Just add them in, tell your LLM to use them as it sees fit, and many of the detailed prompting you did in the past can just disappear.

  Some work now is just getting skills to work reliably in gptel, a solution that is lighter than adding a layer of agent framework.
...
* Overview
  ...
  goals
  + make prompts composable (now pre-empted by skills)
...
* Other interesting resources for agentic workflows and more
  + [[https://github.com/sdi2200262/agentic-project-management/tree/main][sdi2200262/agentic-project-management: A framework for structuring and managing complex projects using multiple AI agents.]]

* LLM system prompts
** Start with this tool use and skills prompt for programming and research


   #+description: A prompt that instructs the LLM to use MCP tools and skills, as well as read Agents.md and similar project-specific prompts
   #+name: tools-skills-project-prompt
...
*** Improve tool selection and usage
    Long lists of tools with short descriptions can be confusing for LLMs.
...
#+end_src
    ...
    Refer to the tool usage prompt at [[https://github.com/gregoryg/AIPIHKAL/blob/main/README.org#improve-tool-selection-and-usage][AIPIKHAL -> Improve tool selection and usage (github)]]

*** Use Skills a la Claude Skills
#+description: Give Skills to LLM
#+name: skills
    #+begin_src gfm
...
*** Advanced: The Council of LLMs
    :PROPERTIES:
    :image:    img/llm-council-portrait-shaman-brains-in-jars-01.jpeg
    :END:
    inspired by an Andrej Karpathy weekend vibe coding tweet.

    This prompt differs from others in this README in that it relies on =mcp-rubber-duck=, which can be found on GitHub at
    [[https://github.com/nesquikm/mcp-rubber-duck][nesquikm/mcp-rubber-duck: An MCP server that acts as a bridge to query multiple OpenAI-compatible LLMs with MCP tool access.]]


    #+description: Council of LLMs
    #+name: llm-council
    #+begin_src gfm
