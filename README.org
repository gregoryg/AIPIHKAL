...
* Update early 2026
  ...
  Skills are prompts - composable not by you the user, but by the LLM.  Every AI platform supports them now, and if yours does not, you can simply tell the LLM where you have some skills in a directory, and the LLM will figure out how to use them.  They're dead simple, and they often leverage either simple MCP servers and/or CLI scripts or APIs.  They do not impose the "LLM cognitive overload" that many MCP servers do, so you end up with an uncluttered context, which means a clear-thinking LLM.  I have just started implementing some skills here, but the reality is that you can find huge lists of useful skills all over the internet.  Just add them in, tell your LLM to use them as it sees fit, and many of the detailed prompting you did in the past can just disappear.
