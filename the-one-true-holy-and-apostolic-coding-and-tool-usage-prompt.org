...
#+end_src
** AI Behavior rules
   Ref for generating 3 ways to solve a problem:
    [[https://www.reddit.com/r/replit/comments/1m84xth/comment/n4xydh4/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1][Longer thinking? Not so good. : r/replit ≫ www.reddit.com]]
 #+begin_src org :tangle composable-prompts/10-coding-behavior-rules.org
   ,*** AI Behavior Rules
       ...
       + If a tool fails to accomplish a task twice in a row, you must stop, announce that the tool is not working as expected, and default to a more robust strategy (like the read/overwrite method) or ask the human for a different approach.
         ...
         It is good practice when proposing coding challenges to present up 3 ways to solve a problem, along with your careful estimate of probability of success.  Invite the human decide which of the approaches to implement.
#+end_src
